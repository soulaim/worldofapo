...
void Level::generate(int seed)
{
  ...
  for(int i=0; i<walls_info.size(); i++)
  {
    ...
    for(int k=0; k<walls_info[i].size(); k++)
      if(rand() % 10 < 2)
        walls_info[i][k] = 1;
  }
...
FixedPoint Level::getHeight(FixedPoint& x, FixedPoint& y)
{
  ...
  int x_desimal = x.getDesimal() * 8;
  int y_desimal = y.getDesimal() * 8;
  
  if(x_index < 0 || x_index > pointheight_info.size()-2)
    return FixedPoint(0);
  if(y_index < 0 || y_index > pointheight_info.size()-2)
    return FixedPoint(0);
