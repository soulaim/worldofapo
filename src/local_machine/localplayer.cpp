...
void Localplayer::handle(const BulletHitEvent& event)
{
	...
	int x = (rand() % 4);
	ss << "hit" << x;
    sendMsg(SoundEvent(ss.str(), 100000, event.t_position));
...
{
    sendMsg(SoundEvent("hit0", 100000, event.t_position));
	int scale = 1000;
	int max_rand = 500;
...
void Localplayer::handle(const DeathPlayerEvent& event)
{
	...
	if(event.target_id == game.myID)
	{
        ...
        sendMsg(SoundEvent("player_death", 100000, world.units[game.myID].getEyePosition()));
	}
	else
	{
        sendMsg(SoundEvent("player_death", 100000, event.t_position));
	}
...
void Localplayer::handle(const DeathNPCEvent& event)
{
	...
	sendMsg(SoundEvent("alien_death", 100000, event.t_position));
...
{
    // TODO: This is just stupid and ugly.
	if(game.myID >= 0)
